...
# Run aider in architect mode with specific model settings
aider:
	uv run aider --architect --model o3-mini --reasoning-effort high --editor-model o3-mini 

# Run aider with the oneshot shell script
# Usage: make aider-oneshot PROMPT=path/to/your/prompt.txt
aider-oneshot:
	@if [ -z "$(PROMPT)" ]; then \
		echo "Error: PROMPT is required. Usage: make aider-oneshot PROMPT=path/to/your/prompt.txt"; \
		exit 1; \
	fi
	@if [ ! -f "$(PROMPT)" ]; then \
		echo "Error: File $(PROMPT) does not exist"; \
		exit 1; \
	fi
	@echo "Running aider oneshot with prompt from $(PROMPT)..."
	@uv run sh "./aider prompt templates/aider_oneshot.sh" "$$(cat $(PROMPT))"

# Run aider with the reflect shell script
# Usage: make aider-reflect PROMPT=path/to/your/prompt.txt
aider-reflect:
	@if [ -z "$(PROMPT)" ]; then \
		echo "Error: PROMPT is required. Usage: make aider-reflect PROMPT=path/to/your/prompt.txt"; \
		exit 1; \
	fi
	@if [ ! -f "$(PROMPT)" ]; then \
		echo "Error: File $(PROMPT) does not exist"; \
	...
	@echo "Running aider reflect with prompt from $(PROMPT)..."
	@uv run sh "./aider prompt templates/aider_reflect.sh" "$$(cat $(PROMPT))" 